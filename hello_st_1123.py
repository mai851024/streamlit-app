...
st.title('Machine Learning Model Comparison for Heart Disease Prediction')
st.markdown('⭐本研究透過數據探索分析（Exploratory Data Analysis, EDA）檢視年齡、性別、胸痛類型及最大心率等特徵與心臟病之間的相關性。同時，應用多種機器學習模型（如邏輯迴歸、隨機森林、XGBoost）進行心臟病風險的預測。結果顯示，胸痛類型（cp）、最大心率（thalach）、運動誘發的心絞痛（exang）為重要的預測變數。在模型性能評估中，邏輯迴歸（LR）在準確率、精確率及F1分數方面表現最佳，展現對正類樣本的高度識別能力，為臨床診斷提供了有價值的參考依據。')
